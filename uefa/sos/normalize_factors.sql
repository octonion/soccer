...
from uefa._parameter_levels
where parameter='year'
);

drop table if exists uefa._factors;

create table uefa._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from uefa._parameter_levels npl
left outer join uefa._basic_factors nbf
...
update uefa._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=uefa._factors.parameter;
